---
title: March 2023 Archive Log
author: "Alexander Wen"
date: "2023-05-30"
date-modified: "2026-07-24"
categories: [competitive programming,2023,archive]
---
...
Well it's good to see in these past logs that Leetcode competitions being won by someone who solves in sub 10 minutes was a thing even before AI became quite significant. Then again, the fact I was able to climb back to 2600 ELO in Leetcode as recently as May 30, 2026 (Weekly Contest 504) shows at least some stability in the rating system.
...
If D1 is counted, then I actually have done a faster 4 problem solve in Div 2 in 27 minutes from [Round 1111](https://codeforces.com/contest/2247/standings/participant/242040955#p242040955). It's more of an astrisk since the overall problems were easier and D2 also existed. D2 itself is probably deserving of another Honestcode post entirely.
...
Somewhat unrelated to this post, but imagine if at the start of each contest, each coder received the problems in a completely random order. The solve stats as the contest goes on would still be shown, but for instance, if the easiest problem was A in someone's set with 8000 solves at a given point, it could be that for another person that easiest problem is shown as problem D with the same 8000 solves. It would then destroy the easiest to hardest assumption in these contests which would be very interesting and possibly lead to fairer problem rating calculations; there are cases where a later Problem D/E/F can be rated much higher mainly because Problem C took up a lot of implementation time.
...
I actually have some live recordings from me doing Codechef on the SnowLucario account in this [playlist](https://www.youtube.com/watch?v=XAPQ-yrUM2M&list=PLJpjxONwMEgMQrHVjN6tHH2rPxMfgwMOn). This is actually how my competitive programming setup looks, so in case you ever thought you needed complex templates/macros for this, you have these live demonstrations of me pretty much just using IDLE as my IDE and Paintbrush (MS Paint equivalent for Mac OS).
