...
Like my previous award winning (but not actually award winning) monologue of the greatest contest in [Polynomial Round 2022](https://alxwen711.github.io/HonestCode/posts/archive/2022-12/#polynomial-round-2022) the final stats of this contest are hidden for dramatic effect.
...
:::{.callout-note}
# Looking Back at the Code Style on D
...
This was from a [Python Competitive Programming Library](https://github.com/alxwen711/pythonCompetitiveLibrary) that I was making previously as an alternative to [kactl](https://github.com/kth-competitive-programming/kactl), which is the C++ codebook. For a while I had been regularly updating it but in actual practice I found that I did not use the codebook I had made very much. Even in the rare times I did use it such as this problem, I only really used the absolute basic aspects of the queue (adding and removal) which would have been much easier and time efficient with just using a simple array and pointer method. This is more due to my own style that has very little use for codebooks in general, and even though I don't personally use the codebook, it is nice to see it does get occasional attention. I definitely know that at least one person has found use in my work as they use my template.
...
This node structure with `list()` rather than `dict()` is fine given that there are at most 2000 nodes, and that a $O(n^2)$ solution was expected. However, `self.ID` was pretty much just the index in the `nodes` list and was never used.
...
In this case `nodes[l].depth` was always going to be positive, so I'm not sure why `else` was not used at the end.
