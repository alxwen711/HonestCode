---
title: September 2022 Archive Log
...
:::{.callout-note}
## A Quick Note on Hashtable Hacks

Given several extra years of dealing with hashtable hacks being a thing (and having been hacked at some point at least one more time), nowadays the hacking system in Codeforces is not much of a factor as even in some Div 2 contests the contest will explicitly say that hacks on the early problems (A-D) are disabled. Knowing that they are hackable though is still useful for interview purposes. Also from experience, there is a tradeoff in the complexity of a hashing algorithm and the added amount of time/potential for errors that can occur; I have been able to get away with very simple hash alterations such as adding and multiplying fixed values to the hash values and this is sufficient since when these testcases are made to break hashtables, they almost always only test base implementations.
:::
...
![hahahahahahahahaha.](hackdefence.png)
...
:::{.callout-note}
## Disclaimer on the Whole Car Crash Analogy
...
:::{.callout-note}
The above video link is broken now, but since the Internet is forever ([and TheTechRobo made a project that can retrive original videos from deleted Youtube links](https://github.com/TheTechRobo/youtubevideofinder/)), we have [this](https://youtu.be/nUWmbL28g50?si=2HaCxREgu7odr9g6) as a fine suitable replacement.
