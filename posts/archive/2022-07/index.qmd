...
![4 contests in a week.](CodeforcesSuperWeek.png)
...
:::{.callout-note}
## Editor's Note on the C System Fail

[https://codeforces.com/contest/1701/submission/163272117](https://codeforces.com/contest/1701/submission/163272117) was the initial submission that ended up system failing on test case 15. From analysis the idea is actually reasonable; from what I could tell I was simulating the entire task list properly by first assigning proficient tasks on each worker if available, then if no proficient tasks when left then to consider adding in a suplementary task, prioritizing tasks with the highest frequency left. This solution actually could work in $O(n)$, but the problem is in these lines of implementation:
...
For a better overall description on segment trees, check out the [second written post on HonestCode](https://alxwen711.github.io/HonestCode/posts/2026/database-segment-tree/).
...
Goes without saying, but please don't code `Tree` like this. Even in a competitive programming setting I have no clue why I had to also put all the variables in one line.
