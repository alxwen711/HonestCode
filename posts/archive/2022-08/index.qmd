...
:::{.callout-tip}
## [Empty Graph](https://codeforces.com/contest/1712/problem/D) Upsolve in 2026
...
1. No value in the array is less than `x/2`. Other wise any path for nodes a to b can be shorter than `x` via a `a -> j -> b` case.
2. There must exist `1 <= i <= n-1` such that `A[i] >= x and A[i+1] >= x`. This ensures a path between two nodes must be at least `x`.
...
![I got the miku rank lol](39.png)
